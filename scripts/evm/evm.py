...
parser.add_argument("--nfuse",
                    ...
                    default=4)
parser.add_argument("--margin_scale",
                    type=float,
...
# BEGIN: ADAPTED FUNCTIONS TO RUN EVM WITH ONLINE DATA

def update_params_small(n_tailsize,
                        n_cover_threshold,
                        n_num_to_fuse,
                        distance):
    global tailsize,cover_threshold,cdist_func,pdist_func,num_to_fuse
    tailsize = n_tailsize
    cover_threshold = n_cover_threshold
    num_to_fuse = n_num_to_fuse
...
def update_fit_fast_helper(newpoints,newlabels,weibulls,points,labels):
    """
    Fit Weibulls only for new points.
    """
    all_points = np.vstack((points,newpoints))
    all_labels = np.concatenate((labels,newlabels))
    d_mat = 0.5 * cdist_func(newpoints,all_points)
    row_range = range(len(d_mat))
    args = zip(d_mat,row_range,[all_labels for i in row_range])
    with timer("...getting weibulls"):
        p = Pool(num_thread)
        new_weibulls = p.map(weibull_fit_parallel, args)
        p.close()
        p.join()
    weibulls += new_weibulls
    return all_points,all_labels,weibulls

def update_iter(data,labels,update_ind,X,weibulls,y):
    print "...update ind: {}".format(update_ind)
    ind = np.concatenate(tuple([np.where(labels == update_ind[i])[0] for i in xrange(len(update_ind))]))
    pts = data[ind]
    pt_labels = labels[ind]
    X,y,weibulls = update_fit_fast_helper(pts,pt_labels,weibulls,X,y)
    X,weibulls,y = reduce_model(X,weibulls,y,np.unique(pt_labels))
    return X,weibulls,y

def update_iter_single(pt,pt_label,X,weibulls,y):
    X,y,weibulls = update_fit_fast_helper(pt,pt_label,weibulls,X,y)
    X,weibulls,y = reduce_model(X,weibulls,y,pt_label)
    return X,weibulls,y

# END: ADAPTED FUNCTIONS TO RUN EVM WITH ONLINE DATA
